...
class PinkyPatrolNode(Node):
    ...
    # pinky1 전용 웨이포인트
    # Pinky 1: P1 - P2 - P3 - P4 - P5 - P6 - P5 - P1
   ...
   POINTS = {
	...
	# Pinky 2 경로: 원점 아래쪽 -> (P2로 가정, 확인 필요) -> 우측 사각형 -> 원점 복귀
	# WAYPOINTS = [
	#     POINTS["P7"],  # 시작점 (원점 아래쪽, 0.002, -0.597)
	#     POINTS["P2"],  # ⚠ 원본 메모에 번호 누락 — P2로 가정 (0.598, 0.151)
	#     POINTS["P3"],  # 우측 아래 끝 (1.502, -0.400)
	#     POINTS["P4"],  # 중앙-우측 아래 (1.000, -0.400)
	#     POINTS["P5"],  # 중앙-우측 위 (1.000, 0.100)
	#     POINTS["P6"],  # 우측 위 끝 (1.498, 0.100)
	#     POINTS["P1"],  # 원점 부근으로 복귀 (0.002, 0.001)
	# ]
...
def main(args=None):
    ...
    try:
        rclpy.spin(node)   # patrol_cmd 명령을 계속 기다림 (메인 스레드는 여기서 대기)
    finally:
        node.destroy_node()
        rclpy.shutdown()
