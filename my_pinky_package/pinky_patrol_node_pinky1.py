...
class PinkyPatrolNode(Node):
    ...
    "RED": (0.600, -0.600),  # RED LINE — 위험 구역 진입을 표시하는 경계 지점
	}
    ...
    WAYPOINTS = [
        ...
        POINTS["RED"],  # 2: RED LINE (0.600, -0.600)         <- 여기 도착 후 진입 허가 요청
        POINTS["P3"],   # 3: 우측 아래 끝 (1.502, -0.400)       (위험 구역 안)
        POINTS["P6"],   # 4: 우측 위 끝 (1.498, 0.100)          (위험 구역 안)
        POINTS["P1"],   # 5: 시작점으로 복귀 (0.002, 0.001)      <- 여기 도착 시 이탈 통보
    ]
    ...
    이번 경로는 RED에 도착한 뒤 P3로 향하기 직전부터 위험 구역이 시작되고,
    P3 -> P6를 거쳐 다시 P1(원점)으로 돌아오면 위험 구역을 벗어난 것으로 판단합니다:
      - RED(index 2)에 도착한 직후, P3로 출발하기 전에 진입 허가를 받고
      - P1(index 5)에 도착하면 위험 구역을 벗어난 것으로 봅니다.

    * index는 WAYPOINTS 리스트 안에서 그 좌표가 몇 번째(0부터 시작)에 있는지를
      가리키는 값입니다. 실제 좌표 값이 아니라 "리스트에서의 순번"입니다.
    """

    ZONE_ENTRY_INDEX = 2   # WAYPOINTS[2] = RED, 도착 직후 다음 구간(P3)으로 넘어가기 전 진입 허가 대기
    ZONE_EXIT_INDEX = 5    # WAYPOINTS[5] = P1, 여기 도착 시 위험 구역을 완전히 벗어났다고 통보
    # ============================================================================
